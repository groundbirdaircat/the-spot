...
var item = {
    ...
    addToTile( { x, y, under, tag } ){
        ...
        var tile = map.getTile(this.tileX, this.tileY)

        if (!tile) return console.info('tried to add to tile but none found', x, y)

        if (under) {
            this.tag = tag
            tile.getHouseOrLinkedHouse().items.push(this)
        }
        else {
    ...
    removeFromTile(){
        ...
        // handle tile existing in tile's holding items
            ...
            else console.info('tried to remove item from tile but found no index for it')
        }
        // handle tile existing in linked tile's holding items
            ...
            else console.info('LINKED tried to remove item from tile but found no index for it')
        }
    },
    putInPlayerItems(player){
        this.x = 
        this.y = 
        this.tileX = 
        this.tileY = -1
    ...
    pickItemUp(player){
        // called from 'wsclient' -> 'handleItemPickup'
        this.removeFromTile()
        this.itemInWorld = false
        this.putInPlayerItems(player)
    },
    removeFromPlayerItems(player){
        var index = player.items.indexOf(this)

        if (index == -1) return console.info('tried to remove item from player but none found')
...
var mapTile = {
    ...
    removePlayerFromTile(thePlayer){
        ...
        if (index == -1) return console.info('found no player when trying to remove from tile')
...
module.exports = {
    ...
    get getAllItems() { return allItems }
}